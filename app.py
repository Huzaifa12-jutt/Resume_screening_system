...
Layout:
    Sidebar  -> Groq API key (auto-loaded from .env), sample data generator,
                resume upload, job description input, Process button
    Tab 1    -> Rankings (colour-coded table + charts + strengths/weaknesses)
    Tab 2    -> Candidate Profiles (full parsed detail per candidate)
    Tab 3    -> AI Chatbot (Groq / Llama 3.1 8B Instant)

Run locally:   streamlit run app.py
Deploy:        push to GitHub -> streamlit.io/cloud -> set GROQ_API_KEY
               as a secret (see README.md)
"""
...
load_dotenv()  # reads GROQ_API_KEY (and anything else) from a local .env file, if present
...
def inject_css():
    ...
    <style>
        ...
        .hero-sub { color: #9FB3D1; font-size: 15px; }
        .eyebrow {
        ...
        .stButton>button:hover { background: linear-gradient(120deg, #24D9A8, #1AB89E); color: #06140F; }
...
# ---------------------------------------------------------------------------
for key, default in [
    ("candidates", []), ("results", []), ("job_description", DEFAULT_JD),
    ("chat_history", []), ("sample_files", {}),
]:
...
def sidebar():
    with st.sidebar:
        ...
        st.caption("AI-Powered Resume Screening & Candidate Ranking")
        ...
        if env_key and not st.session_state.manual_api_key:
            st.success("✅ Key auto-loaded from environment", icon="🔒")
            api_key = env_key
            with st.expander("Use a different key instead"):
                override = st.text_input(
                    "Override key", type="password", placeholder="gsk_...", key="override_key_input",
                    label_visibility="collapsed",
                )
                if override:
                    st.session_state.manual_api_key = override
                    api_key = override
        else:
            st.caption("No key found in `.env` — paste one below, or add `GROQ_API_KEY` to your `.env` file to skip this.")
            api_key = st.text_input(
                "Groq API Key", value=st.session_state.manual_api_key,
        ...
        uploaded_resumes = st.file_uploader(
            "Multiple PDF resumes", type=["pdf"], accept_multiple_files=True,
        )
        ...
        jd_text = st.text_area("Paste job description", value=st.session_state.job_description, height=150)
        jd_pdf = st.file_uploader("...or upload JD as PDF", type=["pdf"], key="jd_pdf")
        if jd_pdf is not None:
            jd_text = extract_text_from_pdf(jd_pdf)
            st.info("Using text extracted from uploaded JD PDF.")

        st.markdown("---")
        process = st.button("🚀 Process & Rank Candidates", use_container_width=True, type="primary")
...
def process_resumes(uploaded_resumes, include_samples, jd_text):
    ...
    for i, (file_obj, name) in enumerate(files_to_process):
        try:
            file_obj.seek(0)
            candidates.append(parse_resume(file_obj, name))
...
def render_rankings_tab():
    ...
    for r in results:
        df_rows.append({
            "Rank": r["rank"], "Candidate": r["name"], "Overall": r["overall_score"],
            "Skills": r["breakdown"]["skills"], "Experience": r["breakdown"]["experience"],
            "Education": r["breakdown"]["education"], "Certifications": r["breakdown"]["certifications"],
    ...
    st.download_button(
        "⬇️ Export Rankings as CSV", data=df.to_csv(index=False).encode("utf-8"),
    ...
    for cat, color in zip(categories, colors_seq):
        fig.add_trace(go.Bar(
            name=cat, x=[r["name"] for r in results], y=[r["breakdown"][cat.lower()] for r in results],
            marker_color=color,
        ))
    fig.update_layout(
...
def radar_chart(candidates_subset: list):
    """Small reusable radar/spider chart for one or two candidates' category breakdown."""
    categories = ["Skills", "Experience", "Education", "Certifications", "Projects"]
    ...
    for r, color in zip(candidates_subset, palette):
        ...
        fig.add_trace(go.Scatterpolar(
            ...
            fill="toself", name=r["name"], line_color=color, opacity=0.75,
        ))
    fig.update_layout(
        polar=dict(radialaxis=dict(visible=True, range=[0, 100], color="#7C93BF"),
                    bgcolor="rgba(0,0,0,0)"),
        template="plotly_dark", paper_bgcolor="rgba(0,0,0,0)",
        showlegend=len(candidates_subset) > 1, height=260,
...
def render_compare_section(results):
    ...
    c1, c2 = st.columns(2)
    name_a = c1.selectbox("Candidate A", names, index=0, key="compare_a")
    name_b = c2.selectbox("Candidate B", names, index=min(1, len(names) - 1), key="compare_b")
...
def render_chatbot_tab(api_key):
    ...
    if not results:
        ...
        return
    if not api_key:
        st.warning("Enter your Groq API key in the sidebar to enable the chatbot.")
        return
...
def main():
    ...
    if st.session_state.sample_files:
        with st.expander(f"📁 {len(st.session_state.sample_files)} sample resume(s) generated — preview / download"):
            for label, pdf_bytes in st.session_state.sample_files.items():
                st.download_button(f"⬇️ {label}", data=pdf_bytes,
                                    file_name=label.split(" - ")[0].replace(" ", "_") + ".pdf",
                                    mime="application/pdf", key=f"dl_{label}")
